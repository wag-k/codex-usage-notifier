...
FiveHourなどの短期枠では、残量が99%以上に回復し、同じリセット期間について未通知の場合に回復を通知します。300分枠はFiveHour候補ですが、すべてのプランや環境で返されるとは限りません。存在しない場合もエラーにせず、画面に「5時間枠：未観測」と表示します。
...
未使用分が次の利用期間へ繰り越されることは確認できていません。本アプリでは、リセット前に残量を確認できるよう通知します。期限を過ぎたリセット前通知は送りません。

### 長期枠のリセット完了通知

リセット予定時刻を過ぎた後に利用枠を再取得し、`resetsAt`の変化、`usedPercent`の50ポイント以上の低下、または保存済みのリセット期間IDとの差異を確認できた場合に、新しい利用期間の開始を通知します。タイマーが予定時刻へ到達しただけでは通知済み状態を確定しません。

通知対象は自動または手動で選択します。自動選択では300分枠を優先し、存在しなければ観測できた最短の正の期間を持つ枠を選択します。Unknown枠は初期設定では通知対象にしません。
...
Phase 1の基盤とPhase 2のCodex App Server連携に加え、Phase 3の監視・Windows通知まで実装しています。
...
2026年8月4日にCodex CLI `0.145.0-alpha.18`で実アカウントを確認したところ、`limitId=codex`の`primary`に10080分・使用率35%の週間枠だけが存在し、`secondary=null`、リセット回数1、300分枠は未観測でした。この場合、画面には「5時間枠：未観測」と表示し、自動選択の通知対象候補は10080分枠になります。300分枠は単体テストで対応済みですが、すべての環境で取得できるとは限りません。
...
短期枠回復通知、長期枠のリセット前通知・リセット完了通知は初版とPhase 3の実装対象です。
