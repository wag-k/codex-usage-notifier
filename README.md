...
リセット予定時刻を過ぎた後に利用枠を再取得し、`resetsAt`の変化、`usedPercent`の大幅な低下、または保存済みのリセット期間IDとの差異を確認できた場合に、新しい利用期間の開始を通知します。タイマーが予定時刻へ到達しただけでは通知済み状態を確定しません。

通知対象は自動または手動で選択します。自動選択では300分枠を優先し、存在しなければ観測できた最短の正の期間を持つ枠を選択します。通知実装時、Unknown枠は初期設定では通知対象にしません。
...
通知判定、通知段階・通知済み状態の表示、リセットまでの残り時間表示、Windows通知、Gmail通知、履歴グラフは未実装です。観測履歴のJSONL保存は実装済みです。
...
短期枠回復通知、長期枠のリセット前通知・リセット完了通知は初版の対象です。ただし、現在のPhase 2実装には通知判定と通知送信をまだ含みません。
