...
Codex App Serverが返す任意の利用枠をWindows上で観測し、選択した利用枠が十分に回復したときに、WindowsとGmailへ通知する常駐アプリです。
...
- 通知対象を自動選択、またはLimitId・位置・ウィンドウ長で手動選択
- 選択した利用枠の残量が設定した閾値以上になったときに通知
...
将来の通知実装では、初期設定で次の条件を満たしたときに通知します。

1. 自動選択した通知対象の残量が99%以上
2. 同じ回復期間について未通知
3. 通知禁止時間外であること

自動選択では300分枠を優先し、存在しなければ観測できた最短期間の枠を選択します。5時間枠通知は300分枠を取得できる環境で有効になります。300分枠が存在しないこと自体はエラーではありません。
...
2026年8月4日の実アカウント確認では、`limitId=codex`の`primary`に10080分の週間枠だけが存在し、`secondary=null`、300分枠は未観測でした。この場合、画面には「5時間枠：未観測」と表示し、自動選択の通知対象候補は10080分枠になります。

通知判定、Windows通知、Gmail通知、履歴グラフは未実装です。観測履歴のJSONL保存は実装済みです。
