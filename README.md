...
リセット予定時刻を過ぎた後に利用枠を再取得し、`resetsAt`が次の期間へ進んだ場合は`ResetTimeAdvanced`、同一利用枠の`usedPercent`が前回の正常取得から50ポイント以上低下した場合は`UsageDropInference`として、新しい利用期間の開始を通知します。`resetsAt`がない場合も使用率低下による推定は可能です。タイマーが予定時刻へ到達しただけでは通知済み状態を確定しません。
...
Phase 1の基盤、Phase 2のCodex App Server連携、Phase 3の監視・Windows通知、およびPhase 3.1の複数枠通知基盤まで実装しています。
...
Gmail通知、設定画面、履歴グラフは未実装です。通知別設定は`settings.json`の内部モデルとして追加済みで、観測履歴のJSONL保存も実装済みです。リセット完了の使用率低下による補助判定は、初期値として50ポイント以上の低下を使用します。
...
利用枠を再取得
   ...
   │      └─ リセット時刻の前進、または50ポイント以上の使用率低下を確認
   │
...
短期枠回復通知、長期枠のリセット前通知・リセット完了通知は初版とPhase 3の実装対象であり、複数枠・回復連番・テスト通知はPhase 3.1で実装しています。
