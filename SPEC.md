...
7. Phase 2では選択結果を候補として表示するが、Unknown除外を含む通知ポリシー、通知判定、通知送信はPhase 3以降に実装する。
...
閾値は1～100%の範囲で設定でき、初期値は99%とする。300分枠を取得できる環境ではFiveHourとして判定できるが、300分枠が存在しない状態も正常とする。Phase 2では通知対象候補を表示するだけで、通知判定と通知送信は行わない。
...
次のモデルはPhase 3以降で通知状態を実装する際の概念モデルであり、Phase 2時点ではクラスを実装しない。
...
次の表は初版完成時の設定概念モデルを示す。Phase 2時点で実装済みなのはCodexExecutablePath、NotificationTarget、および既存の基盤設定までであり、新しい通知別設定はPhase 3以降で追加する。
...
- リセット予定時刻後に利用枠を再取得する。
- `resetsAt`の変化、`usedPercent`の大幅な低下、またはリセット期間IDの変化を確認した場合にリセット完了通知候補になる。
...
6. 配布形式
7. アプリ名・アイコン
8. 短期枠回復通知の保留中に残量が閾値未満へ下がった場合も、回復していた事実を通知するか
9. `usedPercent`の「大幅な低下」と判定する具体的な差分
10. `resetsAt`が取得できない場合のリセット期間ID生成規則
11. Unknown枠を通知対象にした場合、短期回復と長期リセット通知のどちらを適用するか
12. 自動選択で短期枠と長期枠を同時に通知対象とするか、現在実装済みの単一候補選択を維持するか
